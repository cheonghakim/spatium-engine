...
type Drag = { id: string; start: Point; end: Point; origin: Point; handle: 'start' | 'end' | 'body' } | { id: string; vertex: number };

export class DraftReviewTool implements EditorTool {
  readonly id = 'draft-review';
  private drag: Drag | null = null;
  constructor(private readonly context: ToolContext, private readonly draft: DraftManager) {}
  activate(): void { this.context.selection.clear(); }
  deactivate(): void { this.drag = null; this.draft.cancelEdit(); }
  onPointerDown(event: EditorPointerEvent): void {
    ...
    if (selectedWall) {
      ...
      if (handle) {
        this.drag = { id: selectedWall.id, start: { ...selectedWall.start }, end: { ...selectedWall.end }, origin: event.worldPoint, handle };
        this.draft.beginEdit(); return;
      }
    }
    ...
    if (space && vertex >= 0) {
      this.drag = { id: space.id, vertex }; this.draft.beginEdit(); return;
    }
    const walls = state.walls.map(wall => ({ wall, d: distanceToSegment(event.worldPoint, wall.start, wall.end).distance }))
      .filter(hit => hit.d <= radius).sort((a,b) => a.d - b.d);
    const wall = walls[0]?.wall;
    if (wall) {
      this.draft.select(wall.id);
      this.drag = { id: wall.id, start: { ...wall.start }, end: { ...wall.end }, origin: event.worldPoint,
        handle: distance(wall.start, event.worldPoint) <= radius ? 'start' : distance(wall.end, event.worldPoint) <= radius ? 'end' : 'body' };
      this.draft.beginEdit(); return;
    }
    const hit = [...state.spaces].reverse().find(s => isPointInPolygon(event.worldPoint, s.polygon));
  ...
  onPointerMove(event: EditorPointerEvent): void {
    ...
    if (!event.altKey && !('handle' in drag && drag.handle === 'body')) {
      ...
      for (const wall of this.draft.current?.walls ?? []) {
        ...
        for (const candidate of [wall.start, wall.end]) {
          ...
          if (d < best) { point = candidate; best = d; }
        }
      }
    }
    if ('vertex' in drag) { this.draft.updateSpaceVertex(drag.id, drag.vertex, point); return; }
    ...
    if (drag.handle === 'body') {
      ...
      if (event.shiftKey) {
        ...
        point = Math.abs(point.x - anchor.x) > Math.abs(point.y - anchor.y) ? { x: point.x, y: anchor.y } : { x: anchor.x, y: point.y };
      }
      if (drag.handle === 'start') start = point; else end = point;
    }
    this.draft.updateWall(drag.id, start, end);
  }
  onPointerUp(): void { this.drag = null; this.draft.commitEdit(); }
  onKeyDown(event: EditorKeyboardEvent): void {
    if (event.key === 'Escape') { this.deactivate(); return; }
