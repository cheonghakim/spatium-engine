...
describe("architectural geometry", () => {
  it("retains the wall above doors and below windows", () => {
    ...
    const window = {
      ...createEntrance("f", { x: 4, y: 0 }, "window"),
      width: 2,
      height: 1,
      sillHeight: 0.8,
    };
    ...
    const solid = (x: number, y: number) =>
      panels.some((p) => x > p.from && x < p.to && y > p.bottom && y < p.top);
  ...
  it("subtracts overlapping openings as a union, with no negative or duplicate panels", () => {
    ...
    const b = {
      ...createEntrance("f", { x: 2, y: 0 }, "window"),
      width: 2,
      height: 1,
      sillHeight: 1,
    };
    ...
    expect(panels.reduce((sum, p) => sum + (p.to - p.from) * (p.top - p.bottom), 0)).toBeCloseTo(
      9.6 - 4.5,
    );
  });
  it("builds stairs at their actual position, direction, rise, width and landing", () => {
    const stair = {
      ...createEntrance("f", { x: 10, y: 20 }, "stairs"),
      width: 1.5,
      depth: 4,
      height: 3,
      stepCount: 12,
    ...
    expect(model.elements.children[0]?.children.length).toBeGreaterThan(13);
  });
  ...
  it("creates glass and door leaves, including legacy entrances with no dimensions", () => {
    const model = buildArchitecture(
      [],
      [createEntrance("f", { x: 0, y: 0 }), createEntrance("f", { x: 3, y: 0 }, "window")],
    );
    let glass = 0,
      meshes = 0;
    model.elements.traverse((object) => {
      if (object instanceof THREE.Mesh) {
        meshes++;
        if (object.material.transparent) glass++;
      }
    });
