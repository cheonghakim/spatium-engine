...
/** Lets a caller elsewhere in the app (the empty-state "도면 불러오기" shortcut) open the file picker directly, instead of just switching to this tab and leaving the user to find the upload button themselves. */
...
const hasDraft = computed(() => {
  revision.value;
  return props.editor.draft.hasDraft;
});
function preset(mode: "balanced" | "clean" | "faint"): void {
  Object.assign(settings, {
    autoThreshold: true,
    threshold: 128,
    minSegmentLengthPx: mode === "clean" ? 45 : mode === "faint" ? 15 : 25,
    maxLineGapPx: mode === "faint" ? 3 : 2,
    maxMergeGapPx: 2,
    minWallThicknessPx: mode === "clean" ? 3 : mode === "faint" ? 1 : 2,
    snapTolerancePx: 3,
  });
}
...
async function autoVectorize(): Promise<void> {
  ...
  try {
    ...
    if (props.editor.getActiveFloor()?.id !== floorId || props.editor.reference.current !== source)
      return;
    const { threshold, ...rest } = settings;
    const result = vectorizeReferenceImage(props.editor.reference, {
      ...rest,
...
<template>
  <div class="reference-panel">
    ...
    <label class="upload-button">
      이미지 업로드
      <input ref="fileInputRef" type="file" accept="image/png,image/jpeg" @change="onFileChange" />
    </label>
    <p v-if="uploadError" class="error-text" role="alert">
      {{ uploadError }}
    </p>
    ...
    <template v-if="reference">
      ...
      <h3>자동 벡터화</h3>
      <div class="presets">
        <button @click="preset('balanced')">기본</button
        ><button @click="preset('clean')">가구 선 줄이기</button
        ><button @click="preset('faint')">얇은 벽 살리기</button>
      </div>
      <details class="settings">
        <summary>감지 설정 조절</summary>
        <label
          ><input v-model="settings.autoThreshold" type="checkbox" /> 이미지 밝기에 맞춰 자동
          감지</label
        >
        <label v-if="!settings.autoThreshold"
          >밝기 기준 {{ settings.threshold
          }}<input v-model.number="settings.threshold" type="range" min="1" max="254"
        /></label>
        <label
          >최소 선 길이 {{ settings.minSegmentLengthPx }} px<input
            v-model.number="settings.minSegmentLengthPx"
            type="range"
            min="5"
            max="150"
        /></label>
        <label
          >최소 벽 두께 {{ settings.minWallThicknessPx }} px<input
            v-model.number="settings.minWallThicknessPx"
            type="range"
            min="1"
            max="10"
        /></label>
        <label
          >끊긴 선 연결 {{ settings.maxLineGapPx }} px<input
            v-model.number="settings.maxLineGapPx"
            type="range"
            min="0"
            max="8"
        /></label>
        <label
          >이중선 병합 간격 {{ settings.maxMergeGapPx }} px<input
            v-model.number="settings.maxMergeGapPx"
            type="range"
            min="1"
            max="30"
        /></label>
        <p class="hint">
          가구가 잡히면 최소 길이·두께를 높이세요. 벽이 빠지면 낮추세요. 연결·병합 간격을 너무
          높이면 출입구나 서로 다른 벽이 합쳐질 수 있습니다.
        </p>
      </details>
      <p v-if="hasDraft" class="hint">
        다시 분석하면 현재 초안과 초안 수정 내역을 새 결과로 교체합니다. 확정된 지도는 유지됩니다.
      </p>
      <button class="vectorize-button" :disabled="vectorizing" @click="autoVectorize">
        {{ vectorizing ? "분석 중..." : hasDraft ? "다시 분석 (초안 교체)" : "자동 벡터화" }}
      </button>
      <p class="hint">
        수평·수직 벽을 감지합니다. 사선·곡선 벽은 직접 그려 보완해 주세요. 투명도와 화면 격자는 감지
        결과에 영향을 주지 않습니다.
      </p>
      <p class="hint">
        문·창문으로 쓸 벽 사이 틈과 계단 후보도 찾습니다. 먼저 축척을 보정하고, 검토 탭에서 후보
        종류·치수·상행 방향을 확인하세요.
      </p>
      <p v-if="vectorizeError" class="error-text">
        {{ vectorizeError }}
      </p>
