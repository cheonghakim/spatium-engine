...
test("review an opening as a window and replace stair strokes in the 3D preview", async ({
  page,
}, testInfo) => {
  const errors: string[] = [];
  ...
  const dataUrl = await page.evaluate(() => {
    ...
    ctx.fillStyle = "#000";
    ctx.fillRect(20, 20, 280, 4);
    ctx.fillRect(20, 20, 4, 200);
    ctx.fillRect(296, 20, 4, 200);
    ctx.fillRect(20, 216, 120, 4);
    ctx.fillRect(160, 216, 140, 4);
  ...
  await expect(
    review.getByRole("heading", { name: "건축 요소 후보 (0/2)", exact: true }),
  ).toBeVisible();
  ...
  await expect(
    review.getByRole("heading", { name: "건축 요소 후보 (1/2)", exact: true }),
  ).toBeVisible();
  ...
  await review.getByRole("button", { name: "검토한 후보 포함", exact: true }).click();
  await expect(
    review.getByRole("heading", { name: "건축 요소 후보 (2/2)", exact: true }),
  ).toBeVisible();
  ...
  expect(floor.entrances.find((e: { type: string }) => e.type === "window")).toMatchObject({
    sillHeight: 0.8,
  });
  expect(floor.entrances.find((e: { type: string }) => e.type === "stairs")).toMatchObject({
    height: 2.8,
    stepCount: 8,
  });
