...
test('upload, vectorize, edit, exclude, undo, confirm and export', async ({ page }, testInfo) => {
  ...
  const dataUrl = await page.evaluate(() => {
    ...
    ctx.fillStyle = '#eeeeee'; ctx.fillRect(0,0,240,180);
    ctx.strokeStyle = '#444444'; ctx.lineWidth = 4; ctx.strokeRect(20,20,200,140);
    ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(70,90); ctx.lineTo(120,90); ctx.stroke();
  ...
  await page.mouse.down(); await page.mouse.move(bounds.x+bounds.width/2+20,bounds.y+bounds.height/2+20,{steps:5}); await page.mouse.up();
  ...
  await expect(page.getByRole('button',{name:'격자',exact:true})).toHaveAttribute('aria-pressed','false');
  ...
  await page.mouse.move(bounds.x+bounds.width/2,bounds.y+bounds.height/2);
  await page.mouse.down(); await page.mouse.move(bounds.x+bounds.width/2,bounds.y+bounds.height/2+20,{steps:5}); await page.mouse.up();
  const moved = await exportProject();
  expect(moved.buildings[0].floors[0].walls[0].start.y).toBeCloseTo(project.buildings[0].floors[0].walls[0].start.y-0.4);
