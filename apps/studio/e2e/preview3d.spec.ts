...
async function renderedPixels(page: Page): Promise<number> {
  return page.locator('.preview-3d canvas:visible').evaluate((element) => new Promise<number>(resolve => {
...
for (const large of [false, true]) {
  test(`3D renders unconfirmed vectorization and confirmed walls (${large ? 'large off-center plan' : 'normal plan'})`, async ({page},testInfo) => {
    const errors: string[] = [];
    ...
    const dataUrl = await page.evaluate(large => {
      ...
      canvas.width = large ? 2400 : 240; canvas.height = large ? 1600 : 180;
      const ctx = canvas.getContext('2d')!;
      ctx.fillStyle = '#fff'; ctx.fillRect(0,0,canvas.width,canvas.height);
      ctx.strokeStyle = '#000'; ctx.lineWidth = 4;
      if (large) ctx.strokeRect(1900,1000,300,300); else ctx.strokeRect(20,20,200,140);
      return canvas.toDataURL('image/png');
    },large);
    await page.locator('input[type=file]').setInputFiles({name:'plan.png',mimeType:'image/png',buffer:Buffer.from(dataUrl.split(',')[1]!,'base64')});
